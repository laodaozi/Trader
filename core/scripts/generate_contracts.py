...
3. **个股映射**：只填你确信有明确传导关系的 A 股。code 必须 6 位纯数字。**不确定就别填，禁止产出 type="中性" 或 logic="暂无直接映射" 的无效条目**。type 用"受益|受损|弹性"三选一。
...
8. **引号铁律**：JSON 字符串值内禁止 ASCII 双引号 `"`。中文引号必须使用 `「」`（直角引号）。示例：正确"需警惕「估值泡沫化」风险" / 错误"需警惕"估值泡沫化"风险"
9. **中文输出**，但 JSON key 用英文。"""
...
def parse_narrative_response(text: str) -> Optional[dict]:
    ...
    raw = raw.replace("\u201c", "「").replace("\u201d", "」")  # 中文引号→直角引号（避免破坏JSON结构）

    # 预处理：修复中文引号（LLM 在 JSON 字符串值内用 ASCII " 做中文引号）
    for _ in range(5):  # 安全上限，处理多个中文引号对
        new_raw = re.sub(
            r'([一-鿿])"([一-鿿][^"]{0,200}[一-鿿])"([一-鿿，。；：！？、)）]】　])',
            '\\1「\\2」\\3',
            raw
        )
        if new_raw == raw:
...
def validate_and_fix_narrative(narrative: dict) -> dict:
    ...
    for ev in narrative.get("events", []):
        ...
        # 过滤无效个股映射 (type=中性 或 logic=暂无直接映射)
        clean2 = []
        for s in clean:
            stype = str(s.get("type", "")).strip()
            slogic = str(s.get("logic", "")).strip()
            if stype == "中性" or "暂无直接映射" in slogic or "无直接映射" in slogic:
                print(f"    ⚠ 剔除无效个股: {s.get('name','?')} (type={stype}, logic={slogic[:15]})")
                continue
            clean2.append(s)
        ev["stock_mapping"] = clean2
        if not ev.get("stock_mapping"): ev["stock_mapping"] = []

    # ETF weight 归一化 + 过滤 weight=0 空筹码
    etfs = narrative.get("etf_allocation", [])
    if etfs:
        total = sum(e.get("weight", 0) for e in etfs if isinstance(e.get("weight"), (int, float)))
        if total > 0 and abs(total - 1.0) > 0.01:
            for e in etfs:
                e["weight"] = round(e.get("weight", 0) / total, 3)
        # 剔除 weight=0 的无效 ETF（Claude 填了名字但没给仓位）
        etfs = [e for e in etfs if e.get("weight", 0) > 0]
        narrative["etf_allocation"] = etfs
        if etfs:
            # 重新归一化（剔除后剩余的 weight 重新分配）
            total2 = sum(e.get("weight", 0) for e in etfs)
            if total2 > 0:
                for e in etfs:
                    e["weight"] = round(e.get("weight", 0) / total2, 3)
